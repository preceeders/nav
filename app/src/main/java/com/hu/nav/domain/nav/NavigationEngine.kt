...
class NavigationEngine(
    ...
    fun speakCurrentLocation() {
        ...
        val guide = if (path != null && remain != null) {
            path.spokenGuides().getOrNull(path.guideIndexForRemain(remain))
        } else {
            null
        }
        val text = buildString {
            if (guide != null && guide.instruction.isNotBlank()) {
        ...
        naviClient.events.collect { event ->
            when (event) {
                is NaviSdkEvent.Tick -> {
                    ...
                    _state.update {
                        it.copy(
                            remainMeters = event.tick.routeRemainMeters,
                            remainSeconds = event.tick.routeRemainSeconds,
                            currentRoad = event.tick.currentRoadName,
                            nextRoad = event.tick.nextRoadName,
                        )
                    }
                    dispatch(planner.onTick(event.tick))
                }
                ...
                NaviSdkEvent.SdkYaw -> {
                    if (_state.value.running) dispatch(planner.onSdkYaw())
                }
        ...
        compass.headings().collect { heading ->
            ...
            _state.update {
                it.copy(
                    ...
                    expectedHeadingDegrees = planner.expectedHeading,
                )
            }
            dispatch(planner.onCompass(heading.toDouble(), lastTick))
        }
    }
    ...
    private fun dispatch(events: List<GuideEvent>) {
        events.forEach { event ->
            ...
            _state.update { it.copy(phase = planner.phase, hint = hintOf(event)) }
            speak(event)
    ...
    private fun requestReroute() {
        ...
        val from = lastTick?.location ?: origin ?: return
        rerouteInFlight = true
        scope.launch {
            naviClient.calculateWalkRoutes(from, dest.location)
        }
    }
